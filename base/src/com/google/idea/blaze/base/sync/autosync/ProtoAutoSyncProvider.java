...
class ProtoAutoSyncProvider implements AutoSyncProvider {
  ...
  public BlazeSyncParams getAutoSyncParamsForFile(Project project, VirtualFile modifiedFile) {
    ...
    return plainTargets.isEmpty()
        ? null
        : new BlazeSyncParams.Builder(AUTO_SYNC_TITLE, SyncMode.PARTIAL)
  ...
  private static List<Label> getPlainTargets(BlazeProjectData projectData, TargetKey target) {
    ...
    todo.addAll(projectData.reverseDependencies.get(target));
    ...
    while (!todo.isEmpty()) {
      ...
      if (targetKey.isPlainTarget()) {
        ...
        todo.addAll(projectData.reverseDependencies.get(targetKey));
      }
    }
    return output;
  }
