...
public class FastBuildJavaCompilerExtensionPoint implements FastBuildCompilerExtensionPoint {

  //Kept with previous class in case people have configured the logger, can consider removing
  ...
  public FastBuildJavaCompilerExtensionPoint(Supplier<EventLoggingService> eventLoggerSupplier, Supplier<File> fastBuildJavacJarSupplier) {
    this.eventLoggerSupplier = eventLoggerSupplier;
    this.fastBuildJavacJarSupplier = fastBuildJavacJarSupplier;
  }
  public FastBuildJavaCompilerExtensionPoint() {
    this(
        EventLoggingService::getInstance,
        FastBuildJavaCompilerExtensionPoint::findFastBuildJavacJar);
  }
  public FastBuildCompiler getCompiler(List<File> javacJars, List<File> bootJars, String sourceVersion, String targetVersion)
      throws FastBuildException {
    Javac javac = createCompiler(javacJars);
    return new JavacRunner(
        javac, bootJars, sourceVersion, targetVersion);
  }

  @Override
  public boolean canCreateCompilerFor(Set<File> filesToCompile) {
    return filesToCompile.stream().allMatch(f -> f.getName().endsWith(".java"));
  }
  ...
  private static File findFastBuildJavacJar() {
    IdeaPluginDescriptor blazePlugin =
        PluginManager.getPlugin(
            PluginManager.getPluginByClassName(FastBuildCompilerFactoryImpl.class.getName()));  //TODO
